...
## 1. Goals
- Show **settlement-canonical actuals** from Wunderground RKSI daily history (current observation + day-high, whole °C).
- Show **30-minute METAR actuals** (AWC only) for comparison and accuracy tracking.
...
- **Charts:** line chart with AWC actuals; optional overlay of forecast models later.
...
- **Local dev:** Supabase Local config + migrations live in `supabase/` (see `supabase/config.toml` + `supabase/migrations/`).

## 3. Data Sources
### 3.1 Settlement Actuals (Wunderground)
- **Provider:** Wunderground daily history (RKSI / Incheon).
- **Endpoint (HTML):** `https://www.wunderground.com/history/daily/kr/incheon/RKSI/date/YYYY-M-D`
- **Reason:** this is the **resolution source** for the Polymarket market; treat it as canonical.
- **Parsed fields:** latest observation time/temp + day-high/day-low (convert to °C).
...
### 3.4 Forecasts (Hourly, decision support)
...
- **Providers:**
  - **AviationWeather.gov (free, no key):** RKSI **TAF** (forecast product).
    ...
    - Note: the METAR endpoint is **observations only** (see §3.2). TAF is not strictly an hourly temperature forecast; store it as raw + parsed features for later use.
...
### 5.3 Day High So Far
- Compute **running max** from WU observations since local 00:00 (persisted), and reconcile to the WU Almanac **Actual** High once it finalizes.
...
- `GET /api/dashboard`
  - Returns a single payload for the frontend containing:
    ...
    - `weather`: WU canonical actuals + 30-minute AWC actual array for comparison.
  ...
  - Weather payload details:
    ...
    - `weather.wunderground`: canonical daily-history snapshot (current, dayHigh/dayLow, and whole °C bucket).
      - `weather.wunderground.observedMaxC`: running max from WU observations (since 00:00 KST).
      - `weather.wunderground.observedMaxCelsiusWhole`: rounded whole °C bucket of the running max.
    - `weather.sources`: latest/dayHigh for AWC plus deltas vs WU.
      - `weather.sources.awc.latestObservedAt`: latest METAR observation timestamp (KST ISO).
...
- **Actuals:** Wunderground daily-history scrape (settlement) + METAR last 24h from Aviation Weather Center.
...
- **Wunderground daily history (HTML):** 300s (`WU_TTL_SECONDS=300`)
...
- **On each poll cycle (cadence depends on upstream):**
  ...
  - Insert `weather_metar_obs` **only when a new observation timestamp appears** (enforced by unique constraint).
  - Insert `weather_wu_obs` (current obs snapshot) and update `weather_day_high_changes` when the WU day-high (whole °C) increases.
...
- **Header:**
  ...
  - Last refresh time
  - Day high so far
  - Current “bucket now” (rounded whole °C) and time left in KST day
- **Weather panel:**
  - Line chart: AWC actuals with hover tooltip (time + temperature) and labeled axes.
  - WU “settlement” card showing current + day-high/day-low (whole °C bucket).
  - Latest observed AWC temp + staleness (minutes since last observation)
- **Market panel:**
  - Summary strip:
    - Market-implied mean temperature (normalized from Yes mid prices) + top implied bucket
    - Forecast daily max (default model) + ensemble range across models
    - Current winning bucket from day-high so far + delta to next bucket
  - Table of outcomes: label, Yes/No **bid/ask**, spread + top-of-book size; highlight outcome matching day high so far
- **Notifications:**
...
- `AWC_TTL_SECONDS=60`
- `WU_HISTORY_HOST=https://www.wunderground.com`
- `WU_TTL_SECONDS=300`
...
create table if not exists weather_wu_obs (
  id bigint generated by default as identity primary key,
  station text not null default 'RKSI',
  observed_at timestamptz not null,
...
create index if not exists idx_weather_wu_obs_time
  on weather_wu_obs (station, observed_at desc);

create table if not exists weather_day_high_changes (
  id bigint generated by default as identity primary key,
  station text not null default 'RKSI',
  source text not null, -- "wunderground" | "wunderground_observed"
  date_kst date not null,
  observed_at timestamptz not null,
